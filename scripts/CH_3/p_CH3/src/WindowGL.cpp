...
void WindowGL::init(){
    ...
    glGenVertexArrays(numVAOs, vao); //Creates VAOs an associate with that identifier (type) GLuint
    glBindVertexArray(vao[0]); //Make that VAO the one that is active
...
void WindowGL::display(double currentTime){
    ...
    GLuint angleLoc = glGetUniformLocation(renderingProgram, "angle"); // get ptr to "offset"
	glProgramUniform1f(renderingProgram, angleLoc, 90.0f);
    //glProgramUniform1f(renderingProgram, offsetLoc, x);
    
    //scaleVec
    GLuint scaleLoc = glGetUniformLocation(renderingProgram, "scaleVec"); // get ptr to "offset"
	glUniform3f(scaleLoc, 3.0f, 3.0f, 3.0f); // no scale by default
