...
void Window::Initialize(){
    ...
    m_custom = new CustomModel("assets/objs/Teapot.obj");
    SetupVertices();
    ...
    m_model_vertices = m_custom->getNumVertices();
...
void Window::SetupVertices(){
    ...
    std::cout<<"Vertices: "<<m_custom->getPValues().size()<<'\n';
    
    // VBO for texture coordinates
    if(m_custom->getTValues().size()>0){
            std::cout<<"Vertices: "<<m_custom->getTValues().size()<<'\n';
            glBindBuffer(GL_ARRAY_BUFFER, m_vbo[1]);
    ...
    if(m_custom->getNValues().size()>0){
            std::cout<<"Vertices: "<<m_custom->getNValues().size()<<'\n';
            glBindBuffer(GL_ARRAY_BUFFER, m_vbo[2]);
...
void Window::RenderCustom(){
    ...
    m_camera->GetViewMatrixByRef() *= glm::rotate(glm::mat4(1.0f), static_cast<float>(glfwGetTime()), glm::vec3(0.0f, 1.0f, 0.0f));
    // m_camera->GetViewMatrixByRef() = glm::scale(m_camera->GetViewMatrixByRef(), glm::vec3(8.0,8.0,8.0));
    ...
    glDrawArrays(GL_POINTS, 0, m_model_vertices);


    //model *= glm::rotate(glm::mat4(1.0f), angle, axis);

    // glDrawArrays(GL_LINES, 0, m_custom->getNumVertices());
    // glDrawArrays(GL_LINES, 0, m_custom->getNumVertices());
